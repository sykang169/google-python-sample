   ...
   "tools": [
     ...
     {"name": "get_brokerage_fee", "svc": "GetOfficialNoticeInfoService", "op": "getStockTradingFeeInfo",
      "doc": "증권사 주식거래 수수료 공시를 조회한다. 가격 경쟁 포지션 확인용.\n\n"
             ...
             "(삼성증권 오프라인이 전 구간 0.497% 정률로 맞아떨어진다.)\n"
             "trAmt를 만원 단위로 읽으면 요율 계산이 전부 틀린다.\n\n"
             "**행마다 basDt가 다르다. 표 전체에 기준일 하나를 붙이지 않는다.**\n"
             "회사가 수수료를 바꾼 날이 곧 basDt이고 회사마다 몇 년씩 벌어진다\n"
             "(실측: 미래에셋 2025-03-04, 삼성증권 2021-10-01, 키움 2020-12-31).\n"
             "회사별 최신 basDt만 남겨 비교하고, **공시일이 벌어진 사실을 먼저\n"
             "밝힌다.** 한 회사의 공시일을 표 전체 기준일로 적으면 4~5년 된 값이\n"
             "현재 수수료로 제시된다.\n\n"
             ...
             "같은 개설 경로 안에서도 요율이 10배까지 갈린다(실측: KB증권\n"
             "은행개설계좌가 HTS 0.015%, 스마트폰 0.120%, ARS 0.150%다).\n"
             "'은행개설은 저율'처럼 뭉뚱그리면 스마트폰 이용자에게 틀린 안내가 된다.\n\n"
             "요율을 요약할 때는 **구간별로 다른지 확인한다.** 한 구간 값으로\n"
             "'정률'이라고 단정하지 않는다(실측: 키움 HTS는 10만원 0.010%,\n"
             "50만원 0.014%, 100만원 이상 0.015%로 갈린다)."},
