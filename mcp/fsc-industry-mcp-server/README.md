...
| `get_fund_code` | 펀드 표준코드를 조회한다. 판매 상품 마스터이자 식별의 출발점. |
| `get_fund_sales` | 펀드 판매현황을 조회한다. 고객유형(개인/일반법인/금융법인)별 판매 |
| `get_securities_firm_stats` | 증권사 일반현황(임직원·점포 등)을 조회한다. |
| `get_bank_stats` | 국내은행 주요경영지표를 조회한다. BIS비율·연체율 같은 건전성 지표. |
| `get_brokerage_fee` | 증권사 주식거래 수수료 공시를 조회한다. 가격 경쟁 포지션 확인용. |
| `get_kofia_stat` | 금융투자협회 종합통계 중 **CMA 현황**을 조회한다. 운용대상 |
...
이 서버가 도는 데 필요한 것은 아래 15건이고, **각각 따로 승인되어 있어야
