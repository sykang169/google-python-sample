...
**여러 계열을 한 표에 놓으면 기준일도 여럿이다.** 계열·회사·항목마다 수록된
최신 시점이 다르다. 표 전체에 기준일 하나를 붙이면 **덜 온 쪽이 실제보다
최신인 것처럼 읽힌다.** 각각의 최신 시점을 확인하고, 다르면 행이나 열에
따로 적는다.
...
**두 수치를 비교하기 전에 모집단이 같은지 확인한다.** "A에서 B로 나아졌다",
"A보다 B가 크다"는 **같은 대상을 센 값끼리**만 할 수 있는 말이다. 한쪽에만
결측이 있거나 한쪽이 일부만 집계한 것이면, 차이가 실제 변화가 아니라 **모집단
차이에서 나온 착시**가 된다.

집계된 값을 쓸 때는 **몇 개를 더한 것인지 확인한다.** 결측이 섞인 합계는 값이
있는 것만 더한 것이다. 모집단이 다르면 **양쪽에 있는 것만으로 다시 계산하고**,
무엇을 뺐는지 밝힌다.
