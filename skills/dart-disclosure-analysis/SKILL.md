...
| status | 뜻 | 대응 |
| --- | --- | --- |
| `013` | 조회된 데이터 없음 | 실패가 아니다. 연도를 한 해 낮추거나 `reprt_code`를 바꿔 재시도 |
| `020` | 분당 1,000건 요청 제한 초과 | 잠시 후 재시도. 반복 조회는 배치를 줄인다 |
| `100` | 필드 값 부적절 | `search_dart_apis`로 명세 재확인 |
| `101` | 존재하지 않는 엔드포인트 | 이름을 지어냈다는 신호 |
| `800` | 시스템 점검 | 재시도 |

`013`을 "데이터가 없습니다"로 그대로 사용자에게 돌려주기 전에, 최소 한 번은
직전 사업연도로 낮춰 재시도한다. 이게 실제로 대부분의 원인이다.
